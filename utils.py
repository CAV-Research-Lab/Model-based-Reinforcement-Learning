...
def do_env_rollout(start_state, act_list):
    """
    ...
    N = len(act_list)
    for i in range(N):
        e.set_env_state1(qp =start_state['qp'], qv = start_state['qv'])
        obs = []
        act = []
        rewards = []
        env_infos = []
...
def generate_paths(start_state, N, base_act, filter_coefs, base_seed):
    """
    ...
    for i in range(N):
        ...
        act_list.append(act)
    paths = do_env_rollout(start_state, act_list)
    return paths
...
def gather_paths_parallel(start_state, base_act, filter_coefs,
                          base_seed, paths_per_cpu, num_cpu=None):
