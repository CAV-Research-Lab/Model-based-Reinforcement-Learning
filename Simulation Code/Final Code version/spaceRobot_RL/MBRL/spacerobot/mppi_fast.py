...
class MPPI:
    ...
    def __init__(self, dynamics, running_cost, nx, noise_sigma, num_samples=100, horizon=15, device="cpu",
        ...
        if noise_mu is None:
            noise_mu = 0.05*torch.ones(self.nu, dtype=self.dtype)
...
def run_mppi(mppi, env, retrain_dynamics, retrain_after_iter=50, iter=200, render=True):
    ...
    for i in range(iter):
        ...
        state = np.hstack((state[7:14], state[20:27]))
        # print('state:', state)
